...
if [[ -z "$TARGET_RELEASE_DIR" || -z "$TARGET_SLOT" || -z "$TARGET_PROJECT" || -z "$TARGET_API_PORT" || -z "$TARGET_DIRECTUS_PORT" ]]; then
...
candidate_compose() {
  ...
  compose_file="$(runtime_compose_file "$TARGET_RELEASE_DIR")"
