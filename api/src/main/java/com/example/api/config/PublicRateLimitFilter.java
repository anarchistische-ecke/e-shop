...
public class PublicRateLimitFilter extends OncePerRequestFilter {
    ...
    private RatePolicy resolvePolicy(HttpServletRequest request) {
        ...
        if (path.equals("/chat/telegram/webhook") && method.equals(HttpMethod.POST.name())) {
            return new RatePolicy("chat-webhook", webhookLimit, webhookWindowSeconds);
        }
        if (path.equals("/chat/conversations") && method.equals(HttpMethod.POST.name())) {
            return new RatePolicy("chat-start", chatStartLimit, chatStartWindowSeconds);
        }
        if (path.startsWith("/chat/conversations/")) {
            return new RatePolicy("chat-message", chatMessageLimit, chatMessageWindowSeconds);
        }
